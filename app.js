/**
 * POKER PRO ANALYTICS - Core Engine (V4.3 Stable)
 */

// ==========================================
// INICIALIZAÇÃO SUPABASE (SEGURA)
// ==========================================
...
// ==========================================
// UTILITÁRIOS (Toasts, Validação e RNG)
// ==========================================
...
function setLoadingState(buttonId, isLoading) {
    ...
    if (isLoading) {
        ...
        btn.innerText = btn.getAttribute('data-original-text') || 'Enviar';
...
function mostrarAbaAuth(aba) {
    ...
    if (!formLogin || !formSignup || !tabLogin || !tabSignup) return;
...
async function fazerCadastro() {
    if (!supabase) return showToast("Modo offline: Conexão com banco falhou.", "error");

    const nome = document.getElementById('signup-nome').value.trim();
    const email = document.getElementById('signup-email').value.trim();
    const senha = document.getElementById('signup-senha').value;

    if (!email || !nome || !senha) return showToast("Preencha todos os campos.", "error");
    if (!validarEmail(email)) return showToast("E-mail inválido.", "error");
    if (senha.length < 6) return showToast("Senha deve ter no mínimo 6 caracteres.", "error");
    ...
    if (error) {
        return showToast(error.message === 'User already registered' ? 'E-mail já cadastrado.' : error.message, "error");
    }
    ...
    showToast("Conta criada! Pode entrar.", "success");
...
async function fazerLogin() {
    ...
    if (error || !data.user) {
        return showToast("E-mail ou senha incorretos", "error");
    }
...
function navegarApp(modulo) {
    ...
    const btnNav = document.getElementById(`nav-${modulo}`);
    ...
    const secModulo = document.getElementById(`module-${modulo}`);
    if(secModulo) secModulo.classList.add('active-module');
}

// ==========================================
// MÓDULO 2: BANKROLL MANAGER (CLOUD + FALLBACK)
// ==========================================
...
async function registrarSessao() {
    ...
    let novaSessao = { 
        user_id: AppState.usuario ? AppState.usuario.id : null, 
        local: local, buy_in: buyIn, cash_out: cashOut, lucro: cashOut - buyIn, horas: horas 
    };

    setLoadingState('btn-sessao', true);

    if (AppState.usuario && supabase) {
        const { data, error } = await supabase.from('sessoes').insert([novaSessao]).select();
        if (!error && data) {
            AppState.sessoes.push(normalizarSessao(data[0]));
        } else {
            console.warn("Falha no DB, usando fallback local.");
    ...
    setLoadingState('btn-sessao', false);
...
function calcularEAtualizarTela(filtroLocal) {
    ...
    sFiltradas.forEach(s => { 
        lTotal += Number(s.lucro); invTotal += Number(s.buy_in); hTotais += Number(s.horas); historico.push(lTotal); 
    });
...
function exportarCSV() {
    ...
    AppState.sessoes.forEach(s => {
        const d = new Date(s.data_sessao);
        const dataFormatada = `${d.getDate().toString().padStart(2, '0')}/${(d.getMonth()+1).toString().padStart(2, '0')}/${d.getFullYear()}`;
        csvContent += `${dataFormatada},${s.local},${s.horas},${s.buy_in},${s.cash_out},${s.lucro}\n`;
    });
...
window.onload = async function() {
    try {
        if (!supabase) {
            console.warn("Inicializando sem Supabase. Funcionalidades offline.");
            document.getElementById('auth-screen').classList.remove('hidden');
            return;
        }
